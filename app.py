...
st.set_page_config(page_title="Quantificateur WB V12.1", layout="wide")
st.title("🔬 Quantificateur 2D : Éditeur de Boîtes Natif")
st.markdown("Utilisez l'**Auto-détection**, puis ajustez ou ajoutez des boîtes avec le **Viseur Vert**.")
...
if uploaded_file is not None:
    ...
    with col3:
        ...
        target_h = st.slider("Épaisseur de la bande", 2, 100, 20)
        ...
        if len(st.session_state.saved_boxes) > 0:
            box_to_delete = st.selectbox("Sélectionner une boîte", range(len(st.session_state.saved_boxes)), format_func=lambda i: f"Boîte {i+1} (Y: {st.session_state.saved_boxes[i]['y']})")
    ...
    with col2:
        ...
        # --- NOUVEAUTÉ : Le contrôle manuel du zoom ---
        affichage_largeur = st.slider("🔍 Taille de l'aperçu", 50, 600, 200, help="Ajustez pour voir l'image entière sans scroller.")
        ...
        # Affichage avec la largeur définie par l'utilisateur
        st.image(display_img, caption="Rouge = Sauvegardé | Vert = Viseur", width=affichage_largeur)
